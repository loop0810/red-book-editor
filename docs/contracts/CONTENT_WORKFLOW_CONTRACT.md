...
- `raw_material` 是用户提供的粗略内容素材和事实来源，必填；可以是关键词、流水账、口语化片段或不完整描述，不要求先写成可发布正文。服务端必须保留其中的关键事实和限制条件，但不得把未提供的经历、结果或个人体验写成已发生事实，也不得把原始句子当作最终正文直接复用。
...
- 至少一个标题候选明确体现 `focus`；
- 正文必须重新组织原始素材，开头不得直接复制用户输入；可以增加连接句、结构、钩子和中性总结，但不得虚构具体经历；
...
原文复用检查与事实覆盖检查是两个不同维度：事实可以通过自然改写保留，正文不得通过复制原始段落来满足长度或事实覆盖要求。连续原文复用超过质量阈值时，服务端应要求重新组织；短主题、数字、专有名词和必要事实片段不应单独触发复用错误。
