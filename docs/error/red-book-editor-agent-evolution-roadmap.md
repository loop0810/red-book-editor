...
- 人工评分规则；
- 自动硬失败检测和标签；
- Agent 运行失败码、诊断计数和失败记录；
- baseline 汇总报告。

仍缺少：

- 可量化的自动事实覆盖率和质量阈值；
- Prompt、Profile、模型版本和评测配置的绑定记录；
- Token 和成本统计；
- 最新 baseline 的完整人工评分、最终修改稿和自动对比结果；
- 可阻断 CI 或发布流程的质量门禁命令。
...
### 第 4 轮：客户端 AI Native（已完成；建议历史和精确 Diff 仍可增强）
...
### 第 5 轮：Memory / RAG / MCP（暂缓）

只在 P1-06 完整质量门禁和 P1-04 图片素材链路稳定之后开始。
...
1. `P1-06`：已具备版本绑定、自动事实覆盖率、Token/成本记录、人工评分完整性校验和阻断式质量门禁；最新 schema 4 候选已完成运行和门禁验证但未通过，且已清理，下一步需先修复安全/事实边界与 Agent 收敛问题后重跑；
2. `P1-04`：Agent 对图片素材的实际理解和素材绑定。

下一步先完成 P1-06 的质量门禁和可观测性，再评估图片理解与素材绑定，之后再决定是否进入 RAG、MCP 或 Memory。自动发布仍不在范围内。
